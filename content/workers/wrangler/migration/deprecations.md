...
- Wrangler will no-longer maintain routes added in the dashboard if you have a `route` or `routes` key in your `wrangler.toml`. To manage routes via the dashboard only, remove any `route`/`routes` keys from your `wrangler.toml` and add `workers_dev = false`.
